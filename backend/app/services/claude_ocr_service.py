...
Шукай фрази:
- "Партія:", "Партія №:", "Номер партії:", "Batch:", "Lot:"
- Формати: дата (17.09.2025), код (ABC123), "Партія співпадає з датою виробництва"
- Шукати в УСЬОМУ тексті етикетки
- Якщо написано що партія співпадає з датою - знайди цю дату (виробництва або "Вжити до")

Приклади:
- "Партія №: 17.09.2025" → "17.09.2025"
- "Batch: ABC123" → "ABC123"
- "Партія співпадає з датою виробництва: 17.09.2025" → "17.09.2025"
- Якщо не знайдено → null
...
## 5a. ALLERGENS (Алергени в складі)
КРИТИЧНО ВАЖЛИВО: Шукати ТІЛЬКИ в розділі "Склад" або "Інгредієнти"

Відомі алергени (14 категорій):
- соя, soy
- молоко, milk
- глютен, gluten
- яйця, яйце, egg
- риба, fish
- ракоподібні, crustaceans
- арахіс, peanut
- горіхи, горіх, nut
- селера, celery
- гірчиця, mustard
...
Якщо знайдено алерген в складі → додай в масив
Якщо немає алергенів → поверни []

Приклад:
Склад: "цитрат магнію, крохмаль кукурудзяний, соя лецитин"
→ allergens: ["соя"]

## 5b. ALLERGEN STATEMENT (Фраза про алергени)
Шукати фразу:
- "Містить алергени:"
- "Продукт містить алергени:"
- "Алергени:"
- "Увага! Містить алергени"

Повернути повний текст фрази як є
Якщо немає фрази → null

Приклад:
"Містить алергени: соя" → "Містить алергени: соя"
...
{{
  ...
  "operator": {{
    ...
    "edrpou": null,
    "address": "Україна, Дніпропетровська обл., м. Дніпро, вул. Сонячна Набережна, буд. 2",
    "phone": "+38(097)106-32-75" або null
  }},
  "manufacturer": {{
    "name": "ТОВ «Біо Лайт»",
    "address": "Україна, Запорізька обл., м. Запоріжжя, вул. Перемоги, буд. 135-А"
  }} або null,
  "batch_number": "17.09.2025" або null,
  ...
  "allergens": ["соя"] або [],
  "allergen_statement": "Містить алергени: соя" або null,
  "mandatory_phrases": {{
